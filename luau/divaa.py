...
if __name__ == "__main__":
    ...
    beta = args.num_envs // 2  # Initialize beta to half the number of environments
    ...
    for update in range(1, num_updates + 1):
        ...
        for step in range(args.num_steps):
            ...
            with torch.no_grad():
                action_s, logprob_s, _, value_s = agent.get_action_and_value(next_obs[:beta])
                action_t, logprob_t, _, value_t = teacher_model.get_action_and_value(next_obs[beta:])
                values[step] = torch.cat((value_s, value_t)).flatten()
                action = torch.cat((action_s, action_t))
                logprob = torch.cat((logprob_s, logprob_t))
            actions[step] = action
        ...
        with torch.no_grad():
            next_value_s = agent.get_value(next_obs[:beta]).reshape(1, -1).squeeze(0)
            next_value_t = teacher_model.get_value(next_obs[beta:]).reshape(1, -1).squeeze(0)
            next_value = torch.cat((next_value_s, next_value_t))
        ...
        # update student
        student_obs = obs[:beta].reshape((-1), *observation_shape)
        student_logprobs = logprobs[:beta].reshape(-1)
        student_actions = actions[:beta].reshape((-1, *envs.single_action_space.shape))
        student_advantages = advantages[:beta].reshape(-1)
        student_returns = returns[:beta].reshape(-1)
        student_values = values[:beta].reshape(-1)

        # Optimizing the policy and value network
        b_inds = np.arange(len(student_obs))
        clipfracs = []
        for _ in range(args.update_epochs):
            rng.shuffle(b_inds)
            for start in range(0, len(student_obs), args.minibatch_size):
                end = start + args.minibatch_size
                mb_inds = b_inds[start:end]
                _, newlogprob, entropy, newvalue = agent.get_action_and_value(student_obs[mb_inds], student_actions.long()[mb_inds])
                logratio = newlogprob - student_logprobs[mb_inds]
                ratio = logratio.exp()
                with torch.no_grad():
                ...
                mb_advantages = student_advantages[mb_inds]
                if args.norm_adv:
                ...
                kl_div = f.kl_div(student_logprobs[mb_inds], newlogprob, reduction="batchmean", log_target=True)
                # Value loss
                newvalue = newvalue.view(-1)
                if args.clip_vloss:
                    v_loss_unclipped = (newvalue - student_returns[mb_inds]) ** 2
                    v_clipped = student_values[mb_inds] + torch.clamp(
                        newvalue - student_values[mb_inds],
                        -args.vf_clip_coef,
                        args.vf_clip_coef,
                    )
                    v_loss_clipped = (v_clipped - student_returns[mb_inds]) ** 2
                    v_loss_max = torch.max(v_loss_unclipped, v_loss_clipped)
                    v_loss = 0.5 * v_loss_max.mean()
                else:
                    v_loss = 0.5 * ((newvalue - student_returns[mb_inds]) ** 2).mean()
        ...
        # update teacher
        teacher_obs = obs[beta:].reshape((-1), *observation_shape)
        teacher_logprobs = logprobs[beta:].reshape(-1)
        teacher_actions = actions[beta:].reshape((-1, *envs.single_action_space.shape))
        teacher_advantages = advantages[beta:].reshape(-1)
        teacher_returns = returns[beta:].reshape(-1)
        teacher_values = values[beta:].reshape(-1)

        # Optimizing the teacher policy and value network
        b_inds = np.arange(len(teacher_obs))
        for _ in range(args.update_epochs):
            rng.shuffle(b_inds)
            for start in range(0, len(teacher_obs), args.minibatch_size):
                end = start + args.minibatch_size
                mb_inds = b_inds[start:end]
                _, newlogprob, entropy, newvalue = teacher_model.get_action_and_value(teacher_obs[mb_inds], teacher_actions.long()[mb_inds])
                logratio = newlogprob - teacher_logprobs[mb_inds]
                ratio = logratio.exp()
                with torch.no_grad():
                    # calculate approx_kl http://joschu.net/blog/kl-approx.html
                    old_approx_kl = (-logratio).mean()
                    approx_kl = ((ratio - 1) - logratio).mean()
                    clipfracs += [((ratio - 1.0).abs() > args.clip_coef).float().mean().item()]
                mb_advantages = teacher_advantages[mb_inds]
                if args.norm_adv:
                    mb_advantages = (mb_advantages - mb_advantages.mean()) / (mb_advantages.std() + 1e-8)
                # Policy loss
                pg_loss1 = -mb_advantages * ratio
                pg_loss2 = -mb_advantages * torch.clamp(ratio, 1 - args.clip_coef, 1 + args.clip_coef)
                pg_loss = torch.max(pg_loss1, pg_loss2).mean()
                kl_div = f.kl_div(teacher_logprobs[mb_inds], newlogprob, reduction="batchmean", log_target=True)
                # Value loss
                newvalue = newvalue.view(-1)
                if args.clip_vloss:
                    v_loss_unclipped = (newvalue - teacher_returns[mb_inds]) ** 2
                    v_clipped = teacher_values[mb_inds] + torch.clamp(
                        newvalue - teacher_values[mb_inds],
                        -args.vf_clip_coef,
                        args.vf_clip_coef,
                    )
                    v_loss_clipped = (v_clipped - teacher_returns[mb_inds]) ** 2
                    v_loss_max = torch.max(v_loss_unclipped, v_loss_clipped)
                    v_loss = 0.5 * v_loss_max.mean()
                else:
                    v_loss = 0.5 * ((newvalue - teacher_returns[mb_inds]) ** 2).mean()
                entropy_loss = entropy.mean()
                loss = pg_loss - args.ent_coef * entropy_loss + v_loss * args.vf_coef
                if args.kl_loss:
                    loss = loss + args.kl_coef * kl_div
                ...
                loss.backward()
                nn.utils.clip_grad_norm_(teacher_model.parameters(), args.max_grad_norm)
                teacher_optimizer.step()

        if rewards[:beta].mean() > rewards[beta:].mean():
            beta = min(beta + 1, args.num_envs)
        else:
            beta = max(beta - 1, 1)
