...
if __name__ == "__main__":
    ...
    h_t = 0
    actions_collected = []  # for logging the actions collected
    ...
    for global_step in range(args.total_timesteps):
        ...
        actions_collected.append(actions.squeeze())
    ...
    # Store to a CSV file
    with Path("diaa_actions.csv").open(mode="w", newline="") as f:
        writer = csv.writer(f)
        writer.writerow(["throttle", "steering"])  # header
        for throttle, steering in actions_collected:
            writer.writerow([throttle, steering])
