...
if __name__ == "__main__":
    ...
    def update_main(data: any) -> TensorDict:
        ...
        critic_proportion = 0.8
        mask1 = torch.rand(b, device=device) < critic_proportion
        mask2 = torch.rand(b, device=device) < critic_proportion

        q1_pred = batched_qf(student_qnet_params[0], data["observations"], data["actions"])
        q2_pred = batched_qf(student_qnet_params[1], data["observations"], data["actions"])

        loss_q1 = f.mse_loss(q1_pred.view(-1)[mask1], next_q_value[mask1])
        loss_q2 = f.mse_loss(q2_pred.view(-1)[mask2], next_q_value[mask2])
        qf_loss = loss_q1 + loss_q2
