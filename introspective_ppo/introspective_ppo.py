...
class PPOIntrospective:
    ...
    def update_critic(self, teacher_correction, rolloutbuffer):
        ...
        discounted_reward = 0
        for reward, is_terminal in zip(
            reversed(rolloutbuffer.rewards), reversed(rolloutbuffer.is_terminals)
        ):
            if is_terminal:
                discounted_reward = 0
            discounted_reward = reward + (self.gamma * discounted_reward)
        ...
        # convert list to tensor
        old_states = (
            torch.squeeze(torch.stack(rolloutbuffer.states, dim=0)).detach().to(device)
        )
        old_direction = (
            torch.squeeze(torch.stack(rolloutbuffer.direction, dim=0))
            .detach()
            .to(device)
        )
        old_actions = (
            torch.squeeze(torch.stack(rolloutbuffer.actions, dim=0)).detach().to(device)
        )
        old_logprobs = (
            torch.squeeze(torch.stack(rolloutbuffer.logprobs, dim=0))
            .detach()
            .to(device)
        )
        old_state_values = (
            torch.squeeze(torch.stack(rolloutbuffer.state_values, dim=0))
            .detach()
            .to(device)
        )
